defmodule VPVersion do
  @moduledoc ~S"""
  Functions for parsing and matching versions against requirements.

  A version is a string in a specific format or a `VPVersion`
  generated after parsing via `VPVersion.parse/1`.

  `VPVersion` parsing and requirements follow
  ...
  The version is represented by the VPVersion struct and it has its
  ...
  @type requirement :: String.t | VPVersion.Requirement.t
  ...
  defmodule InvalidVPVersionError do
    defexception [:message]
  end
  ...
  Raises a `VPVersion.InvalidRequirementError` exception if `requirement` is not
  parseable, or `VPVersion.InvalidVPVersionError` if `version` is not parseable.
  If given an already parsed version and requirement this function won't
  raise.

  ## Examples

      iex> VPVersion.match?("2.0.0", ">1.0.0")
      true

      iex> VPVersion.match?("2.0.0", "==1.0.0")
      false

      iex> VPVersion.match?("foo", "==1.0.0")
      ** (VPVersion.InvalidVPVersionError) foo

      iex> VPVersion.match?("2.0.0", "== ==1.0.0")
      ** (VPVersion.InvalidRequirementError) == ==1.0.0
  ...
  def match?(version, %Requirement{matchspec: spec}) do
    ...
    {major, minor, patch, pre} = to_matchable(version)
  ...
  Raises a `VPVersion.InvalidVPVersionError` exception if `version` is not parseable.
  If given an already parsed version this function won't raise.

  ## Examples

      iex> VPVersion.compare("2.0.1-alpha1", "2.0.0")
      :gt

      iex> VPVersion.compare("2.0.1+build0", "2.0.1")
      :eq

      iex> VPVersion.compare("invalid", "2.0.1")
      ** (VPVersion.InvalidVPVersionError) invalid
  ...
  @doc """
  Parse a version string into a `VPVersion`.

  ## Examples

      iex> {:ok, version} = VPVersion.parse("2.0.1-alpha1")
      iex> version
      #VPVersion<2.0.1-alpha1>

      iex> VPVersion.parse("2.0-alpha1")
      :error

  """
  @spec parse(String.t) :: {:ok, t} | :error
  def parse(string) when is_binary(string) do
    case VPVersion.Parser.parse_version(string) do
      {:ok, {major, minor, patch, pre}} ->
        vsn = %VPVersion{major: major, minor: minor, patch: patch,
  ...
  @doc """
  Parse a version requirement string into a `VPVersion.Requirement`.

  ## Examples

      iex> {:ok, req} = VPVersion.parse_requirement("== 2.0.1")
      iex> req
      #VPVersion.Requirement<== 2.0.1>

      iex> VPVersion.parse_requirement("== == 2.0.1")
      :error

  """
  @spec parse_requirement(String.t) :: {:ok, Requirement.t} | :error
  def parse_requirement(string) when is_binary(string) do
    case VPVersion.Parser.parse_requirement(string) do
      {:ok, spec} ->
  ...
  def to_matchable(%VPVersion{major: major, minor: minor, patch: patch, pre: pre}) do
  ...
  def to_matchable(string) do
    case VPVersion.Parser.parse_version(string) do
      {:ok, vsn} -> vsn
      :error -> raise InvalidVPVersionError, message: string
    end
  end
  ...
  defmodule Parser do
    ...
    @spec parse_requirement(String.t) :: {:ok, VPVersion.Requirement.t} | :error
    ...
    defp component_to_int("*"), do: :any
    defp component_to_int(c), do: String.to_integer(c)

    @spec parse_version(String.t) :: {:ok, VPVersion.matchable} | :error
    def parse_version(string, approximate? \\ false) when is_binary(string) do
      if parsed = Regex.run(@version_regex, string) do
        ...
        if is_nil(minor) or (is_nil(patch) and not approximate?) do
          ...
          major = component_to_int(major)
          minor = component_to_int(minor)
          patch = patch && component_to_int(patch)
    ...
    defp ignore_wildcard(major, minor) do
      {:orelse, 
        {:andalso, 
          {:'==', {{:'$1', :'$2'}}, {:const, {major, minor}}},
          {:'==', {{:'$3'}}, {:const, {:any}}}
        },
        {:andalso,
          {:'==', {{:'$1'}}, {:const, {major}}},
    ...
    defp to_condition([:'==', version | _]) do
      ...
      {:orelse, {:'==', :'$_', {:const, version}}, ignore_wildcard(major, minor)}
    end
...
defimpl String.Chars, for: VPVersion do
...
defimpl Inspect, for: VPVersion do
  def inspect(self, _opts) do
    "#VPVersion<" <> to_string(self) <> ">"
  end
end

defimpl String.Chars, for: VPVersion.Requirement do
  def to_string(%VPVersion.Requirement{source: source}) do
    source
  end
end

defimpl Inspect, for: VPVersion.Requirement do
  def inspect(%VPVersion.Requirement{source: source}, _opts) do
    "#VPVersion.Requirement<" <> source <> ">"
  end
end
